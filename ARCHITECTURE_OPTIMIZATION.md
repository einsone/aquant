...
1. ✅ **Query Service**（问题 2）：4 小时，策略能力提升 50%
1. ✅ **TradingRules 抽象**（问题 7）：6 小时，支持期货/期权
1. ✅ **RiskManager 风控层**（问题 8）：8 小时，真实交易必备

### 中期重构（1-2 周）

1. **DDD 拆分 Portfolio**（问题 1）：需要大量测试确保兼容性
1. **Event Sourcing**（问题 3）：需要设计存储格式
1. **DataManager 窗口缓存**（问题 5）：需要测试性能提升

### 长期演进（1-3 月）

1. **Actor 模型并行**（问题 6）：架构变动大，需要重新设计状态管理
1. **六边形架构重构**：适合 2.0 大版本
