...
def run_single_backtest(strategy_config: StrategyConfig, data_source: ALDSDataSource, backtest_config: BacktestConfig) -> tuple[str, dict[str, Any], float]:
    """运行单个回测
...
def run_parallel_backtest(strategy_configs: list[StrategyConfig], backtest_config: BacktestConfig, max_workers: int = 4) -> dict[str, dict[str, Any]]:
    """并行运行多个策略回测
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_strategy = {executor.submit(run_single_backtest, config, data_source, backtest_config): config for config in strategy_configs}
        ...
        for future in concurrent.futures.as_completed(future_to_strategy):
            ...
            try:
                ...
                results[name] = {"metrics": metrics, "duration": duration, "description": config.description}
...
def compare_strategies(results: dict[str, dict[str, Any]]):
    ...
    # 按收益率排序
    sorted_results = sorted(results.items(), key=lambda x: x[1]["metrics"].get("total_return", 0), reverse=True)
    ...
    for name, data in sorted_results:
        ...
        print(f"{name:<20} {total_return:>11.2f}% {sharpe:>12.2f} {max_drawdown:>11.2f}% {win_rate:>9.1f}% {duration:>10.2f}")
...
def example_parallel_backtest():
    ...
    strategy_configs = [
        StrategyConfig(name="动量策略", strategy=MomentumStrategy(data_source), description="买入过去 20 日涨幅最大的股票"),
        StrategyConfig(name="等权重策略", strategy=EqualWeightStrategy(data_source), description="等权重持有 3 只股票"),
        StrategyConfig(name="买入持有", strategy=BuyAndHoldStrategy(data_source), description="买入单只股票并持有"),
    ]
