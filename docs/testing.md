...
1. **test_data_manager.py** - 数据管理器测试
   - 数据加载
   - 数据缓存
   - 数据预处理

2. **test_query.py** - 查询服务测试
   - 持仓查询
   - 历史数据查询
   - 交易记录查询
...
2. **test_trading_rules.py** - 交易规则测试
   - 涨跌停限制
   - 停牌检查
   - 交易时间验证
...
1. **test_order_manager.py** - 订单管理器测试（21个测试用例）
    - 订单创建与提交
    - 订单状态跟踪
    - 订单撤销
    - 批量操作
...
2. **test_alerts.py** - 告警系统测试（17个测试用例）
    - 多渠道告警（邮件、钉钉、企业微信）
    - 告警管理器
    - 异常处理
    - 批量发送
